defmodule Parser.LexerTest do
  ...
  test "lexing strings that start with numbers" do
    assert matches Lexer.tokenize("0fcaec64-0792-4826-8637-9a50593a7c03", 5), [types([:string]),
                                                                            text(["0fcaec64-0792-4826-8637-9a50593a7c03"])]
  end

  test "lexing strings that contain commas" do
    assert matches Lexer.tokenize("name,email,username", 5), [types([:string]), text(["name,email,username"])]
  end
  ...
  test "lexing option variable" do
    assert matches Lexer.tokenize("--$abc", 5), [types([:string]), text(["--$abc"])]
    assert matches Lexer.tokenize("-$abc", 5), [types([:string]), text(["-$abc"])]
    assert matches Lexer.tokenize("--$ABC", 5), [types([:string]), text(["--$ABC"])]
    assert matches Lexer.tokenize("-$ABC", 5), [types([:string]), text(["-$ABC"])]
    assert matches Lexer.tokenize("--$AbC12", 5), [types([:string]), text(["--$AbC12"])]
    assert matches Lexer.tokenize("-$AbC12", 5), [types([:string]), text(["-$AbC12"])]
    assert matches Lexer.tokenize("-$A", 5), [types([:string]), text(["-$A"])]
  end

  test "lexing long numeric options" do
    assert matches Lexer.tokenize("--foo=123", 5), [types([:string]),
                                                    text(["--foo=123"])]
    assert matches Lexer.tokenize("--foo=123.33", 5), [types([:string]),
                                                    text(["--foo=123.33"])]
  end

  test "lexing long string-like options" do
    assert matches Lexer.tokenize("--foo=abc", 5), [types([:string]), text(["--foo=abc"])]
    assert matches Lexer.tokenize("--foo=\"foo@bar\"", 5), [types([:string, :string]), text(["--foo=", "\"foo@bar\""])]
    assert matches Lexer.tokenize("--foo='foo bar'", 5), [types([:string, :string]), text(["--foo=", "\'foo bar\'"])]
    assert matches Lexer.tokenize("--foo=test:test", 5), [types([:string]), text(["--foo=test:test"])]
  end

  test "lexing long variable options" do
    assert matches Lexer.tokenize("--foo=$bar", 5), [types([:string]), text(["--foo=$bar"])]
  end
  ...
  test "lexing long string-like optvars" do
    assert matches Lexer.tokenize("--$foo=abc", 5), [types(:string)]
    assert matches Lexer.tokenize("--$foo=\"foo@bar\"", 5), [types([:string, :string])]
    assert matches Lexer.tokenize("--$foo='foo bar'", 5), [types([:string, :string])]
    assert matches Lexer.tokenize("--$foo=test:test", 5), [types([:string])]
  end

  test "lexing long variable optvars" do
    assert matches Lexer.tokenize("--$foo=$bar", 5), [types([:string])]
  end

  test "lexing short numeric options" do
    assert matches Lexer.tokenize("-f 123", 5), [types([:string, :integer])]
    assert matches Lexer.tokenize("-f 123.33", 5), [types([:string, :float])]
  end

  test "lexing short string-like options" do
    assert matches Lexer.tokenize("-f abc", 5), [types([:string, :string])]
    assert matches Lexer.tokenize("-f \"foo@bar\"", 5), [types([:string, :string])]
    assert matches Lexer.tokenize("-f 'foo bar'", 5), [types([:string, :string])]
    assert matches Lexer.tokenize("-f test:test", 5), [types([:string, :string])]
  end

  test "lexing short variable options" do
    assert matches Lexer.tokenize("-f $bar", 5), [types([:string, :variable])]

  end

  test "lexing short options with assigned numeric values" do
    assert matches Lexer.tokenize("-f=123", 5), [types([:string])]
    assert matches Lexer.tokenize("-f=123.33", 5), [types([:string])]
  end

  test "lexing short options with assigned string-like values" do
    assert matches Lexer.tokenize("-f=abc", 5), [types([:string])]
    assert matches Lexer.tokenize("-f=\"foo@bar\"", 5), [types([:string, :string])]
    assert matches Lexer.tokenize("-f='foo bar'", 5), [types([:string, :string])]
    assert matches Lexer.tokenize("-f=test:test", 5), [types([:string])]
  end

  test "lexing short options with assigned variable values" do
    assert matches Lexer.tokenize("-f=$bar", 5), [types([:string])]
  end
  ...
  test "lexing short optvars with assigned string-like values" do
    assert matches Lexer.tokenize("-$f=abc", 5), [types([:string]), text(["-$f=abc"])]
    assert matches Lexer.tokenize("-$f=\"foo@bar\"", 5), [types([:string, :string]), text(["-$f=", "\"foo@bar\""])]
    assert matches Lexer.tokenize("-$f='foo bar'", 5), [types([:string, :string]), text(["-$f=", "\'foo bar\'"])]
    assert matches Lexer.tokenize("-$f=test:test", 5), [types([:string]), text(["-$f=test:test"])]
  end
